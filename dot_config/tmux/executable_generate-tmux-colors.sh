...
# Active-pane border COLOR: the pane you're ON matches tmux-dash's active-session
# sidebar border — its theme.active (gnohj_color06, #dc988e; see tmux-dash
# theme.rs where `active` defaults to gnohj_color06 and sidebar.rs draws the
# active row's border_style with it). So the active tmux border and the active
# row in the dash agree. Same on every window; normally the border just means
# "you are here". Copy-mode AND zoom tint it magenta (gnohj_color01).
#
...
active_border_color="#{?@resize_mode,${gnohj_color21},#{?client_prefix,${gnohj_color21},#{?#{||:#{pane_in_mode},#{window_zoomed_flag}},${gnohj_color01},${gnohj_color06}}}}"
...
# thin like the inactive borders but pops by color — the dash-active color
# (gnohj_color06), magenta in copy/zoom. (Filling bg with the color would make a
...
# active-session sidebar border (gnohj_color06) normally, plus the mode-aware
...
# (see pane-border-format in agentic.conf). Both share the copy magenta
# (gnohj_color01): the tint the border itself used to carry, before the
# shared-divider limitation moved it onto the pane's own title line.
set -g @chip_copy "${gnohj_color01}"
...
# @resize_mode session option). A deeper violet (gnohj_color21) so it reads
# distinct from the copy/zoom lavender (@chip_copy) at a glance.
set -g @chip_resize "${gnohj_color21}"
