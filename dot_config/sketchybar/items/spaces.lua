...
local function selectCurrentWorkspace(focusedWorkspaceName)
  for sid, item in pairs(spaces) do
    if item ~= nil then
      local isSelected = sid
        == constants.items.SPACES .. "." .. focusedWorkspaceName
      ...
      if spaceConfig.app then
        ...
        item:set({
          label = {
            color = isSelected and settings.colors.bg1
              or settings.colors.light_blue,
          },
          background = {
            height = 26,
            corner_radius = 9,
            border_width = 2,
            color = isSelected and settings.colors.transparent
              or settings.colors.bg1,
        ...
        item:set({
          ...
          background = {
            ...
            corner_radius = 9,
            border_width = 2,
            color = isSelected and settings.colors.light_blue
              or settings.colors.bg1,
          },
        })
      end
...
local function findAndSelectCurrentWorkspace()
  sbar.exec(
    constants.aerospace.GET_CURRENT_WORKSPACE,
...
swapWatcher:subscribe(constants.events.SWAP_MENU_AND_SPACES, function(env)
  ...
  sbar.set(
    "/" .. constants.items.SPACES .. "\\..*/",
    { drawing = isShowingSpaces }
  )
end)

currentWorkspaceWatcher:subscribe(
