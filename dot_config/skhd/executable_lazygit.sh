...
tmux run-shell '
  ...
  tmux display-popup -E -w 90% -h 90% -d "$DIR" "
    eval \"\$(~/.local/bin/mise activate bash)\" && /usr/bin/git rev-parse --is-inside-work-tree >/dev/null 2>&1 && HUSKY=0 LG_CONFIG_FILE=~/.config/lazygit/config.yml lazygit || exit 0
  "
'
