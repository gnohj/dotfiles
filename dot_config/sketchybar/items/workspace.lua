...
local function updateWorkspaceIndicator()
  ...
  if ok and currentWorkspace then
...
local function updateWindows()
  ...
  -- Get focused window using socket
  local ok3, focusedWindowJson = pcall(function()
    return aerospace:focused_window()
  end)

  if ok3 and focusedWindowJson then
    local cjson = require("cjson")
    local focusedData = cjson.decode(focusedWindowJson)
    if focusedData and #focusedData > 0 then
