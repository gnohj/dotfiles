...
# omits the key entirely when empty, so we need `?` and a default. Don't
# compare timestamps — assertion+invalidation often share a timestamp;
# presence in storeAssertionRecords IS the truth.
state="off"
