...
-- This removes the NVIM_CWD_<pane_id>, NVIM_FILE_<pane_id>, and NVIM_INITIAL_GIT_<pane_id> environment variables so tmux falls back to shell's working directory
...
vim.api.nvim_create_autocmd({ "BufEnter" }, {
  callback = function()
    ...
    -- Only process real files that exist on disk
    if current_file ~= "" and vim.fn.filereadable(current_file) == 1 then
      ...
      if pane_id then
        ...
        -- Store initial git directory as fallback (only set once)
        local initial_git_check = vim.fn.system(
        ...
        if vim.v.shell_error ~= 0 or initial_git_check == "" then
          -- Not set yet, check if current dir is in a git repo
          local git_root = vim.fn.systemlist("git -C " .. escaped_dir .. " rev-parse --show-toplevel 2>/dev/null")[1]
