...
ACTIONS=(
  ...
  "🔁 Sync|🔄 Update Repos|act_sync_update|Pull config repos: chezmoi update + agents||darwin"
  ...
  "🌳 Worktrees|🎫 AI Add Worktree (Chrome tab (jira) → worktree)|act_worktree_jira|Capture the active Chrome Jira tab and create a worktree|mac"
  "🌳 Worktrees|📋 AI Add Worktree (clipboard → worktree)|act_worktree_clipboard|Use clipboard content (text or image) to create a worktree|mac"
  "🌳 Worktrees|🐛 AI Add Worktree (clipboard → Jira bug → worktree)|act_worktree_bug|Classify clipboard as a bug, file Jira ticket, create worktree|mac"
...
# Reuses the zshrc `update` function (chezmoi update + pull agents); mirrors the act_dirty_repos wrapper.
act_sync_update() {
