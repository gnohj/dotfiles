...
echo "Next steps:"
echo ""
echo "  [1] Open a new terminal (Nix + Homebrew paths need a fresh shell)"
echo ""
echo "  [2] Sign in to the App Store, then install the three MAS apps:"
...
echo "      (darwin-rebuild skips these to avoid failing when not signed in)"
echo ""
echo "  [3] Run user-setup.sh to configure dotfiles, secrets, and dev tools:"
echo "        curl -fsSL https://raw.githubusercontent.com/gnohj/dotfiles/main/user-setup.sh | bash -s -- <bitwarden-email>"
echo ""
echo "  [4] Grant TCC permissions after apps are installed"
echo "        System Settings > Privacy & Security"
echo "        See MANUAL_SETUP.md for the full list."
echo ""
echo "Config location: $NIX_CONFIG_DIR"
echo "To re-apply system packages: sudo darwin-rebuild switch --flake $NIX_CONFIG_DIR#$FLAKE_NAME"
echo ""
