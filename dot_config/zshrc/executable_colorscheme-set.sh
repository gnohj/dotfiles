...
generate_starship_config() {
  ...
  os_tux=$(printf '\xf3\xb0\x8c\xbd') # U+F033D nf-md-linux, solid to match the apple's weight
...
add_newline = false
# Raised from the 500ms default: [custom.dir] forks git every render, and a cold pane spawn overran it and printed starship's timeout WARN into the scrollback.
...
"\$schema" = 'https://starship.rs/config-schema.json'
add_newline = false
# Same rationale as the main config - keep the two in sync.
command_timeout = 2000
format = '''
\${env_var.RADIO_CTL}
\${env_var.AWS_PROFILE}
...
customCommands:
  ...
  - key: "P"
    ...
    command: "HUSKY=0 git push --no-verify"
    context: "global"
    subprocess: yes
...
generate_herdr_config() {
  ...
  local herdr_rows
  herdr_rows="$(printf 'rows = [["state_icon", "workspace"], [{ token = "$br", fg = "%s" }, { token = "$br_on", fg = "%s" }, { token = "$git", fg = "%s" }, { token = "$git_on", fg = "%s" }], [{ token = "$pr", fg = "%s" }, { token = "$pr_on", fg = "%s" }, { token = "$ci", fg = "%s" }, { token = "$sb", fg = "%s" }, { token = "$jira", fg = "%s" }], [{ token = "$sys", fg = "%s" }]]' \
    "$gnohj_color13" "$gnohj_color02" "$gnohj_color11" "$gnohj_color02" "$gnohj_color11" "$gnohj_color02" "$gnohj_color03" "$gnohj_color05" "$gnohj_color02" "$gnohj_color46")"
